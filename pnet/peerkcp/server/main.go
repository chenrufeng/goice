...
import (
	...
	"github.com/nkbai/goice/pnet/peerkcp"
)

func main() {

	listener, _ := peerkcp.NewListenPeer(nil, 10, 3)
...
func handlClient(conn *peerkcp.PeerSession) {
	buf := make([]byte, 4096)
	for {
...
func handleEcho(conn *peerkcp.PeerSession) {
	buf := make([]byte, 4096)
	for {
		n, err := conn.Read(buf)
		if err != nil {
